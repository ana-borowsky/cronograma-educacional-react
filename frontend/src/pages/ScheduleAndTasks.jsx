    ...
    <Layout subtitle="Cronograma Semanal">
      <div className="flex flex-col md:flex-row max-w-[1600px] mx-auto p-5 gap-6 items-start">
        ...
        <div className="flex-grow flex flex-col xl:flex-row gap-6">
          ...
          <div className="xl:w-1/4 w-full bg-neutral-800 p-6 border border-neutral-700 rounded-lg shadow-lg flex-shrink-0">
            <h2 className="text-2xl font-bold text-white mb-4 border-b border-neutral-700 pb-2">Tarefas </h2>
            <h3>Ordene por prioridade</h3><br></br>
            <ul className="space-y-3">
              {tasksData.map((task, index) => (
                <li key={index} className={`flex items-start p-3 rounded-md transition duration-200 ${task.completed ? 'bg-neutral-700/50 opacity-60 line-through' : 'bg-neutral-700 hover:bg-neutral-600'}`}>
                  <div className="flex items-center">
                    <input
                      type="checkbox"
                      defaultChecked={task.completed}
                      readOnly
                      className="w-4 h-4 text-blue-500 bg-neutral-700 border-neutral-600 rounded focus:ring-blue-500 cursor-pointer"
                    />
                    <span className={`task-text ml-3 text-sm ${task.completed ? 'text-neutral-500' : 'text-neutral-300'}`}>
                      {task.text}
                      <span className={`text-xs ml-1 font-semibold ${task.completed ? 'text-neutral-500' : 'text-neutral-400'}`}>
                        ({task.estimate})
                      </span>
                    </span>
                  </div>
